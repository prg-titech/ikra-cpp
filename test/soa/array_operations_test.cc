...
TYPED_TEST_P(ArrayTest, ArrayOperations) {
  ...
  Array<Array<int, 3>, kTestSize> arrays;
  ...
  for (int i = 0; i < kTestSize; ++i) {
    Array<int, 3> addition = arrays[i] + arrays[i];
    EXPECT_EQ(array_sum(addition), 2 * array_sum(arrays[i]));
    Array<int, 3> subtraction = addition - arrays[i];
  ...
  for (int i = 0; i < kTestSize; ++i) {
    Array<int, 3> addition = arrays[i] + instances[i]->field1;
    EXPECT_EQ(array_sum(addition), 9*i - 3);
    Array<int, 3> subtraction = instances[i]->field1 - arrays[i];
  ...
  for (int i = 0; i < kTestSize; ++i) {
    Array<int, 3> addition = arrays[i] + instances[i]->field3;
    EXPECT_EQ(array_sum(addition), 6*i);
    Array<int, 3> subtraction = instances[i]->field3 - arrays[i];
  ...
  for (int i = 0; i < kTestSize; ++i) {
    Array<int, 3> addition = instances[i]->field3 + instances[i]->field1;
    EXPECT_EQ(array_sum(addition), 9*i + 3);
    Array<int, 3> subtraction = instances[i]->field1 - instances[i]->field3;
    EXPECT_EQ(array_sum(subtraction), 3*i - 3);
  }

  // Test operations between two AOS array fields.
  for (int i = 0; i < kTestSize; ++i) {
    Array<int, 3> addition = instances[i]->field1 + instances[i]->field1;
    EXPECT_EQ(array_sum(addition), 12*i);
    Array<int, 3> subtraction = instances[i]->field1 - instances[i]->field1;
    EXPECT_EQ(array_sum(subtraction), 0);
  ...
  for (int i = 0; i < kTestSize; ++i) {
    Array<int, 3> addition = instances[i]->field3 + instances[i]->field3;
    EXPECT_EQ(array_sum(addition), 6*i + 6);
    Array<int, 3> subtraction = instances[i]->field3 - instances[i]->field3;
    EXPECT_EQ(array_sum(subtraction), 0);
...
TYPED_TEST_P(ArrayTest, ArrayScalarOperations) {
  ...
  for (int i = 0; i < kTestSize; ++i) {
    Array<int, 3> mul1 = arrays[i] * 3;
    EXPECT_EQ(array_sum(mul1), 9*i - 9);
    Array<int, 3> mul2 = 3 * arrays[i];
  ...
  for (int i = 0; i < kTestSize; ++i) {
    Array<int, 3> mul1 = instances[i]->field1 * 3;
    EXPECT_EQ(array_sum(mul1), 18*i);
    Array<int, 3> mul2 = 3 * instances[i]->field1;
  ...
  // Test multiplications of AOS array fields.
  for (int i = 0; i < kTestSize; ++i) {
    Array<int, 3> mul1 = instances[i]->field3 * 3;
    EXPECT_EQ(array_sum(mul1), 9*i + 9);
    Array<int, 3> mul2 = 3 * instances[i]->field3;
    EXPECT_EQ(array_sum(mul2), 9*i + 9);
